 ...
 */
export async function DetailLocation({
  project,
  locationAsset,
}: {
  project: FullProject;
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-14 sm:px-6">
      ...
      <div className="grid gap-8 md:grid-cols-2">
        <div>
          <p className="text-sm font-medium text-foreground">{t("detail.legalAddress")}</p>
          <p className="mt-1 text-muted-foreground">{address || t("common.unpublished")}</p>
        ...
        <div className="relative aspect-[4/3] overflow-hidden rounded-2xl bg-muted dark:ring-1 dark:ring-border-accent dark:ring-inset">
          {imageUrl ? (
            <ImageWithFallback
              src={imageUrl}
              alt={locationAsset?.alt ?? t("detail.locationMapAlt")}
              fill
              unoptimized
