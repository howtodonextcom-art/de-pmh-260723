    ...
    items: {
      ...
      "04_Hierarchy": {
        ...
        criteria: [
          ...
          { id: "mapBalance", pass: true, note: "map stage md:min-h-[65vh] — Timeline visible same fold @1440" },
          { id: "detailDensity", pass: true, note: "FIXED 2026-08-20 (F22) — SectionDivider rules between chapters, py-16 section rhythm (was py-12), legal dossier accordion collapsed by default; verified via 1440/375 capture, no overflow" },
        ],
        evidence: "luxury-baseline-detail-hh-1440.png",
      },
      "05_Imagery": {
        score: 9.5,
        criteria: [
          ...
          { id: "lcpEager", pass: true, note: "Hero priority + 0 console errors on home capture 2026-08-19" },
          { id: "flipbookGallery", pass: true, note: "AUDIT 2026-08-19 — Issuu-style flipbook fullscreen (toolbar, thumbnail grid, zoom); masonry grid + category tabs retained. UPDATED 2026-08-20 (F07) — reader chrome now reads var(--bg-reader)/var(--fg-reader) tokens instead of hardcoded #111/#ededed, on-brand token layer in place" },
        ],
        evidence: "luxury-flipbook-open-1440.png + luxury-baseline-detail-hh-1440.png",
      },
      "06_Motion": {
        score: 9,
        criteria: [
          { id: "presets", pass: true, note: "lib/motion/presets + hero cascade/kenBurns" },
          { id: "reducedMotion", pass: true, note: "MotionConfig reducedMotion=user sitewide" },
          { id: "luxuryPresence", pass: true, note: "revealUp/stagger home sections + card lift" },
          { id: "flipbookMotion", pass: true, note: "FIXED 2026-08-20 (F06+F05) — FlipbookEngine reads useReducedMotion() and collapses react-pageflip's flippingTime to near-instant; global @media (prefers-reduced-motion: reduce) CSS override also added" },
        ],
        evidence: "components/flipbook/FlipbookEngine.tsx + components/shared/reveal.tsx",
      },
      "07_Mobile": {
        score: 9,
        criteria: [
      ...
      "08_Invisible": {
        ...
        criteria: [
          ...
          { id: "emptyLuxury", pass: true, note: "FIXED 2026-08-20 (F01-F03) — CompareTableSkeleton + LegalLoadingSkeleton (layout-matched, .animate-skeleton pulse) replace plain <p> Suspense fallbacks on /so-sanh and /phap-ly" },
          { id: "flipbookA11y", pass: true, note: "AUDIT 2026-08-19 — role=dialog, aria-modal, toolbar aria-labels, progressbar, Escape closes" },
        ],
        evidence: "app/so-sanh/page.tsx + components/project/detail/project-flipbook-viewer.tsx",
      },
    },
  },
  dimensionB: {
    ...
    items: {
      ...
      B4_PageTransitions: {
        score: 6,
        criteria: [
          { id: "routeFade", pass: true, note: "CORRECTED AUDIT 2026-08-19 — app/template.tsx opacity fade 0.22s on route change (reduced-motion safe)" },
          { id: "continuity", pass: false, note: "Vẫn hard remount feel — không shared-element/layout continuity; chưa luxury editorial flow" },
          { id: "templateOk", pass: true, note: "Opacity-only (no transform) — đúng kỹ thuật, không phá sticky/fixed" },
        ],
      },
      B5_MapCraft: {
        score: 8,
      ...
      B6_EmptyLoading: {
        score: 8,
        criteria: [
          { id: "mapLoading", pass: true, note: "mapLoading i18n + shimmer-sweep visible in map-loading-1440 capture" },
          { id: "skeletonLuxury", pass: true, note: "Teal shimmer-sweep on map + image cards" },
          { id: "errorLuxury", pass: false, note: "error.tsx branded OK; compare/legal empty vẫn utility plain text" },
        ],
      },
    },
  },
    ...
    items: {
      C1_CaptureTooling: {
        ...
        criteria: [
          ...
          { id: "routesCoverage", pass: true, note: "EXPANDED — 15 local views (was 9): +map-loading throttled capture, +/lab, +dark phap-ly, +detail mobile + prod spot. UPDATED 2026-08-20 (F27) — flipbook viewer open state now auto-captured (bespoke visitFlipbookOpen(), clicks a gallery tile and waits for role=dialog) instead of manual-only" },
      ...
      C4_DensityWhitespace: {
        score: 9,
        criteria: [
          { id: "rhythm", pass: true, note: "py-16 section rhythm on home" },
          { id: "flatChrome", pass: true, note: "atmosphere wash + card shadow craft" },
          { id: "cardBreathing", pass: true, note: "FIXED 2026-08-20 (F22) — detail page section padding bumped py-12→py-16 across fact-grid/story/location/masterplan/partners/product-line/amenities/gallery/sales-status/related/sources, SectionDivider rules added between chapters" },
        ],
      },
      C5_DepthAtmosphere: {
        score: 8,
        criteria: [
          ...
          { id: "atmosphere", pass: true, note: "Soft radial teal wash behind Hero (opacity 0.07/0.12)" },
          { id: "material", pass: true, note: "Flipbook dark reader chrome (#111) adds material depth moment; card lift sitewide" },
        ],
      },
    },
  },
  qualitative: {
    balance: "cân đối trên home/catalog; detail page giờ có chapter dividers + py-16 rhythm (F22)",
    color: "teal restrained + amber meta; flipbook giờ dùng var(--bg-reader)/var(--fg-reader) token thay #111/#ededed hardcode (F07)",
    luxuryVsTemplate: "UPDATED 2026-08-20 — Wave 1-4 refactor (F01-F25) landed: skeleton loading states, flipbook reduced-motion + brand token, detail page editorial whitespace, ScopeChip/HeroBlock/Reveal dedup (~1,249 net LOC removed), i18n split-brain fix, vitest + i18n verify script added",
    radiusOveruse: false,
    radiusValue: "0.5rem",
    effects: "Hero cinematic + reveals + flipbook (now reduced-motion guarded); gap: route continuity (B4, deferred P2)",
    perceivedLuxury: 7.3,
    perceivedLuxuryNote: "Estimate per pattern-transfer report's Wave-1 forecast (6.5 → ~7.3-7.5 after skeleton/flipbook-brand/motion fixes) — qualitative, not re-measured by a fresh audit pass",
    completionPct: 80,
    completionNote: "P0+P1 roadmap items from luxury-full-audit.md done (empty-state skeletons, flipbook reduced-motion + brand, detail density/whitespace); P2 items (route continuity, CI hook) still open",
    prodLag: "RESOLVED 2026-08-19 — prod Fraunces H1 + footer + flipbook deployed; hasMinhBach:false",
    deltaVs20260723: "LuxuryIndex 85→87 — detailDensity/flipbookMotion/emptyLuxury/cardBreathing criteria flipped to pass with evidence (F01-F03 skeletons, F06 flipbook reduced-motion, F22 detail rhythm)",
  },
  aggregates: {},
  absoluteS5: {
    currentEstimate: 11,
